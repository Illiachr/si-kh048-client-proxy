...
const openApiFilePath = path.normalize(
  path.join(__dirname, "./specs/si-client-proxy.yml")
);
...
describe("transform response: catalog - base packages", () => {
  it("should satisfy schema in spec", () => {
    ...
    expect(transform(entity, input, envCtx).data[0]).toSatisfySchemaInApiSpec(
      "Package"
    );
  });
});
...
describe("transform response: GET client by ID", () => {
  it("should satisfy schema in spec", () => {
    ...
    expect(transform(entity, input, envCtx).data).toSatisfySchemaInApiSpec(
      "Client"
    );
  });
});
...
describe("transform response: POST /clients", () => {
  it("should satisfy schema in spec", () => {
    ...
    expect(transform(entity, input, envCtx).links).toSatisfySchemaInApiSpec(
      "SelfLink"
    );
  });
});
